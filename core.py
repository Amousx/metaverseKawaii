...
def ExecuteCloudScript(data):
    ...
    while(True):
        ...
        #如果请求调用次数过多 5秒后重试
        if "error" in content and (content['code'] == 429 or content['errorCode'] == 1342):
...
def feed():
    ...
    for key, animalData in playerData["Farm"]["AllAnimals"].items():
        ...
        for index in range(3):#最大喂食次数是3次
            ...
            # print(f'mostFood = {mostFood}')
            # print(mostFood[0])
            # print(foodDict[mostFood[0]])

            if foodDict[mostFood[0]] >= 10:
                animalData["LstFoodIds"].append(mostFood[0])
                foodDict[mostFood[0]] = foodDict[mostFood[0]] - 10
                time.sleep(1)
